...
import ContactCard, {
  ContactDetails,
} from "../components/profileEdit/ContactCard";
import ContactCardEdit from "../components/profileEdit/ContactCardEdit";
import ProjectCard, {
  ProjectDetails,
} from "../components/profileEdit/ProjectCard";
...
import * as helper from "../utils/transformDataHelper";
...
const ProfileEdit: React.FC = (props: any) => {
  ...
  async function updateProfileState() {
    ...
    projectDetails.data = {
      ...projectDetails.data,
      ...helper.getProfileMinistry(ministry.data, projectDetails.data),
      primaryClusterDisplayName: helper.getClusterDisplayName(
    ...
    contactDetails.data = { ...helper.getProfileContacts(contactDetails.data) };
    ...
    setProfileState((profileState0: any) => ({
      ...
      hasPendingEdit,
      isProvisioned: helper.isProfileProvisioned(
        projectDetails.data,
        namespaces.data
      ),
      ...
      quotaDetails: {
        licensePlate: helper.getLicensePlate(namespaces.data),
              ...
              cards.map((c: any, index: number) => (
                <Box key={index}>
                  <Flex
                    p={3}
                    mt={4}
                    bg={theme.colors.bcblue}
                    style={{ position: "relative" }}
                  >
                    <Text as="h3" color={theme.colors.contrast} mx={2}>
                      {c.title}
                    </Text>
                    {!profileState.hasPendingEdit &&
