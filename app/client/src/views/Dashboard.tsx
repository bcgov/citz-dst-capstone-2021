...
const Dashboard: React.FC = () => {
  ...
  //       }
  //       const contactResponses: Array<any> = await Promise.all(
  //         promisesForContact
  //       );
  //       const namespacesResponses: Array<any> = await Promise.all(
  //         promisesForNamespaces
  //       );
  //       const quotaSizeResponse: Array<any> = await Promise.all(
  //         promisesForQuotaSize
  //       );
  //
  ...
  useInterval(() => {
    ...
    Promise.all(promisesForNamespaces).then((namespacesResponses: any) => {
      for (let i: number = 0; i < profile.length; i++) {
        profile[i].provisioned = isProfileProvisioned(
          profile[i],
          namespacesResponses[i].data
        );
      }
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        Cell: ({ row: { values } }: any) =>
          values.provisioned ? "Provisioned" : "Pending",
      },
    ],
    []
  );
