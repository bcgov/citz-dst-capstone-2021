...
export default function useRegistryApi() {
  ...
  const linkContactToProfileById = async (
    ...
    return axiosInstance.current.post(
      `profile/${profileId}/contact/${contactId}`
    );
  };
  ...
  const getContactsByProfileId = async (
    profileId: string
  ): Promise<AxiosResponse<any>> => {
    if (!axiosInstance.current) {
      throw new Error(errorMsg);
  ...
  const getNamespacesByProfileId = async (
    profileId: string
  ): Promise<AxiosResponse<any>> => {
    if (!axiosInstance.current) {
      throw new Error(errorMsg);
  ...
  const getQuotaSizeByProfileId = async (
    profileId: string
  ): Promise<AxiosResponse<any>> => {
    if (!axiosInstance.current) {
      throw new Error(errorMsg);
  ...
  const getAllowedQuotaSizesByProfileId = async (
    ...
    return axiosInstance.current.get(
      `profile/${profileId}/allowed-quota-sizes`
    );
  };
  ...
  const updateQuotaSizeByProfileId = async (
    ...
    return axiosInstance.current.post(
      `profile/${profileId}/quota-size`,
      requstedQuotas
    );
  };

  const getEditRequestStatus = async (
    profileId: string
  ): Promise<AxiosResponse<any>> => {
    if (!axiosInstance.current) {
      throw new Error(errorMsg);
  ...
  const createProjectRequestByProfileId = async (
    profileId: any
  ): Promise<AxiosResponse<any>> => {
    if (!axiosInstance.current) {
      throw new Error(errorMsg);
  ...
  const getHumanActionRequests = async (): Promise<AxiosResponse<any>> => {
    if (!axiosInstance.current) {
      ...
      return axiosInstance.current.get(
        "request/human-action?filter=requires_human_action"
      );
    }
  };
  ...
  const updateProjectRequest = async (
    ...
    if (!axiosInstance.current) {
      ...
      return axiosInstance.current.post(
        `request/${requestId}/human-action`,
        requestBody
      );
    }
  };
